...
export default function EventsScreen() {
  ...
  return (
    <ScreenContainer>
      <TopBar back title="Events" actions={[{ icon: 'cal', onPress: () => toast.info('Coming soon', 'The calendar view is on its way.') }]} />
      {events.map(e => {
        ...
        return (
          <Pressable key={e.id} onPress={() => nav.navigate('EventDetail', { id: e.id })} style={styles.row}>
            <View style={[styles.date, { backgroundColor: e.color + '22' }]}>
              <Text style={[styles.day, { color: e.color }]}>{d.day}</Text>
              <Text style={[styles.month, { color: e.color }]}>{d.month}</Text>
            </View>
            <View style={{ flex: 1 }}>
              <Text style={styles.name} numberOfLines={2} ellipsizeMode="tail">{e.name}</Text>
              <Text style={styles.meta} numberOfLines={1} ellipsizeMode="tail">{d.time}</Text>
